...
$conn1=null;
$arquivo = '{"dbserver": "server","host":"mysql:host=mysql.uhserver.com;dbname=dbnitoli","user":"drsadmin","pwd":"688657Swf@"}';   
$arquivo2 = '{"dbserver": "local","host":"mysql.uhserver.com","db":"dbnitoli","user":"drsadmin","pwd":"688657Swf@"}';   

//atualizar %DEBUG in pedidorel movestrel pedidobaixarel produtorel

function pdoConn() { 
   global $conn1,$arquivo;
   //$arquivo = '{"dbserver": "local","host":"mysql:host=mysql.uhserver.com;dbname=dbnitoli","user":"drsadmin","pwd":"688657Swf@"}';   
   ...
   try { 
      if ($DEBUG) {
         $conn1 = new PDO('mysql:host=localhost;dbname=dbnitoli',"root","");
      }   
      else { 
...
function mysqlconn() { 
    global $conn1,$arquivo2;
    //$arquivo = file_get_contents("server.json");
    
    $json = json_decode($arquivo2);
    $DEBUG=false;
    if ($json->dbserver=="local") {
       $DEBUG = true;
    }   
    if ($DEBUG) {
       $conn1 = mysqli_connect("127.0.0.1","root","","dbnitoli");
    }
    else {
       $conn1 = mysqli_connect($json->host,$json->user,$json->pwd,$json->db);
    }
